        ...
        try {
            ...
            try {
                ...
                const defaultTemplates = [
                    ...
                    {
                        tenant_id: tenant.id,
                        template_key: 'payment_incomplete',
                        name: 'Payment Reminder',
                        description: 'Sent by an admin to remind a member to complete their membership payment',
                        subject: `Complete your ${body.clubName} membership payment`,
                        greeting: 'Hi {{firstName}},',
                        body_intro: `Your ${body.clubName} account is set up, but the membership payment hasn't been completed yet. Once it's done you'll be able to check in to classes straight away.`,
            ...
            // -----------------------------------------------
            // 8. Return success
            // Note: Welcome email is sent by the Stripe webhook
            // when the platform_subscription checkout completes
            // (see webhook/route.ts → renderSubscriptionActivatedEmail)
            // -----------------------------------------------
            return NextResponse.json({
                success: true,
                tenantId: tenant.id,
